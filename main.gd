...
func 사다리풀이그리기() -> void:
	...
	for 참가자번호 in 사다리수.세로줄수:
		...
		var oldy = 0
		# 시작 세로줄 그리기
		사다리자료.풀이이동좌표[참가자번호].append([세로줄위치(현재줄번호,0), 가로줄위치(현재줄번호,0)])
		for y in 사다리수.가로줄수:
			if 사다리자료.자료[현재줄번호][y].왼쪽연결길 == true: # 왼쪽으로 한칸 이동
				# 현재까지의 세로줄 그리기
				사다리자료.풀이이동좌표[참가자번호].append([가로줄위치(현재줄번호,oldy), 가로줄위치(현재줄번호,y)])
				#왼쪽 화살표
				현재줄번호 -= 1
				if 현재줄번호 < 0: # 절반 길이 화살표가 필요
					# 가장 왼쪽
					p1 = 가로줄위치(현재줄번호+1,y)-사다리수.가로화살표위치보정
					p2 = 가로줄위치(현재줄번호,y)-사다리수.가로화살표위치보정
					사다리자료.풀이이동좌표[참가자번호].append([p1, (p1+p2)/2])
					현재줄번호 += 사다리수.세로줄수
					# 가장 오른쪽
					p1 = 가로줄위치(현재줄번호+1,y)-사다리수.가로화살표위치보정
					p2 = 가로줄위치(현재줄번호,y)-사다리수.가로화살표위치보정
					사다리자료.풀이이동좌표[참가자번호].append([(p1 + p2)/2, p2])
				else :
					사다리자료.풀이이동좌표[참가자번호].append([가로줄위치(현재줄번호+1,y)-사다리수.가로화살표위치보정, 가로줄위치(현재줄번호,y)-사다리수.가로화살표위치보정])
				oldy = y
				continue
			if 사다리자료.자료[(현재줄번호+1)%사다리수.세로줄수][y].왼쪽연결길 == true: # 오른쪽으로 한칸 이동
				...
				continue
		# 나머지 끝까지 그린다.
		사다리자료.풀이이동좌표[참가자번호].append([가로줄위치(현재줄번호,oldy), 가로줄위치(현재줄번호,사다리수.가로줄수)-사다리수.세로화살표위치보정])
