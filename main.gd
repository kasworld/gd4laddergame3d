...
func 사다리풀이그리기() -> void:
	...
	for 참가자번호 in 사다리수.세로줄수:
		...
		var oldy = 0
		# 시작 세로줄 그리기
		사다리자료.풀이이동좌표[참가자번호].append([화살표방향.아래쪽, 현재줄번호,0, 0.5])
		for y in 사다리수.가로줄수:
			if 사다리자료.자료[현재줄번호][y].왼쪽연결길 == true: # 왼쪽으로 한칸 이동
				# 현재까지의 세로줄 그리기
				사다리자료.풀이이동좌표[참가자번호].append([화살표방향.아래쪽, 현재줄번호,oldy,y])
				#왼쪽 화살표
				현재줄번호 = (현재줄번호-1+사다리수.세로줄수)%사다리수.세로줄수
				사다리자료.풀이이동좌표[참가자번호].append([화살표방향.왼쪽, 현재줄번호+1, 현재줄번호,y])
				oldy = y
				continue
			if 사다리자료.자료[(현재줄번호+1)%사다리수.세로줄수][y].왼쪽연결길 == true: # 오른쪽으로 한칸 이동
				...
				# 오른쪽 화살표
				현재줄번호 = (현재줄번호+1+사다리수.세로줄수)%사다리수.세로줄수
				사다리자료.풀이이동좌표[참가자번호].append([화살표방향.오른쪽, 현재줄번호-1,현재줄번호,y])
				oldy = y
				continue
		# 나머지 끝까지 그린다.
		사다리자료.풀이이동좌표[참가자번호].append([화살표방향.아래쪽, 현재줄번호,oldy,사다리수.가로줄수])
