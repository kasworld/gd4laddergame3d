		...
		for y in 사다리수.가로줄수:
			if 사다리자료.자료[현재줄번호][y].왼쪽연결길 == true: # 왼쪽으로 한칸 이동
				...
				if 현재줄번호 < 0: # 절반 길이 화살표가 필요
					...
					사다리자료.풀이이동좌표[참가자번호].append_array([가로줄위치(현재줄번호+1,y)-사다리수.가로화살표위치보정, 가로줄위치(현재줄번호,y)-사다리수.가로화살표위치보정])
				oldy = y
				continue
			if 사다리자료.자료[(현재줄번호+1)%사다리수.세로줄수][y].왼쪽연결길 == true: # 오른쪽으로 한칸 이동
				...
				continue
		# 나머지 끝까지 그린다.
		사다리자료.풀이이동좌표[참가자번호].append_array([가로줄위치(현재줄번호,oldy), 가로줄위치(현재줄번호,사다리수.가로줄수)-사다리수.세로화살표위치보정])
